...
export async function POST(req: Request) {
  ...
  const { name, description, startDate, endDate, coverPhoto } = parsed.data

  const trip = await queryOne(
    `INSERT INTO trips (owner_id, name, description, start_date, end_date, cover_photo)
     VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
    [userId, name, description ?? null, startDate, endDate, coverPhoto ?? null]
  )
